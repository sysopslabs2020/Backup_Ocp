...
then
oc login https://console.opencs.domain.it:8443 -u admin -p XXXXXXXX --insecure-skip-tls-verify
elif [ $OSname = "ernvlbastion1.domain.it" ];
then
oc login https://console.cluster01.prod.opencs.domain.it:8443/ -u admin -p XXXXXXXX --insecure-skip-tls-verify
elif [ $OSname = "cpmzbastion1.domain.it" ];
then
oc login https://console.clusterzac-collaudo.opencs.domain.it:8443/console -u admin -p XXXXXXXX --insecure-skip-tls-verify
elif [ $OSname = "epmzbastion1.domain.it" ];
then
oc login https://console.clusterzac1.opencs.domain.it:8443/console -u admin -p XXXXXXXX --insecure-skip-tls-verify
else
...
echo "">>${Backuplog}
  ...
  for object in rolebindings configmap serviceaccounts secrets imagestreamtags cm egressnetworkpolicies rolebindingrestrictions limitranges resourcequotas pvc templates cronjobs statefulsets hpa deployments replicasets poddisruptionbudget endpoints is
  do
echo "">>${Backuplog}
